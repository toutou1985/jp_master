...
- (void)loadDataFromDB
{
    ...
    NSString *tSQL = @"SELECT count() as count FROM game_result gr left join word w on gr.word_id = w.id where  w.mission_id = ? and w.level_id = ? and gr.complete_status != 0";
...
- (void)buttonAciton:(UIButton *)sender
{
    ...
    switch (sender.tag)
    {
        ...
        {
            if ([pointsTaskArr[1][TASK_KEY] isEqualToString:@"6"])
            {
                if (modeType == kModeTypeGame)
                {
                    [self.navigationController pushViewController:gameVC animated:YES];
        ...
        {
            if ([pointsTaskArr[2][TASK_KEY] isEqualToString:@"6"])
            {
                if (modeType == kModeTypeGame)
                {
                    [self.navigationController pushViewController:gameVC animated:YES];
        ...
        {
            if ([pointsTaskArr[3][TASK_KEY] isEqualToString:@"6"])
            {
                if (modeType == kModeTypeGame)
                {
                    [self.navigationController pushViewController:gameVC animated:YES];
