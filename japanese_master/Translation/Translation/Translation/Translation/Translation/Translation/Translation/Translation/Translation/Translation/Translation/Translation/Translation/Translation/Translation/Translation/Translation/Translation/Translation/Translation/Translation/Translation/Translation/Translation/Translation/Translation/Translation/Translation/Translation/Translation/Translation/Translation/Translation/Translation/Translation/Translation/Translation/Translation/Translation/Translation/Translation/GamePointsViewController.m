...
//    if (modeType == kModeTypeGame)
    {
        tSQL = @"select m.game_status ,count(gr.complete_status) as count, m.mission_no from  word w left join  game_result gr on w.id = gr.word_id and gr.complete_status = 1 left join mission m on m.id = w.mission_id where m.mission_no = ?";
    }
